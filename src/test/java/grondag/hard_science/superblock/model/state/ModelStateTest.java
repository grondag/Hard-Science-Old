...
public class ModelStateTest
{
    ...
    public void test()
    {
        HardScience.INSTANCE.info("Max shapes within current format: " + ModelShape.MAX_SHAPES);
        HardScience.INSTANCE.info("bits0 length = "  + ModelStateData.PACKER_0.bitLength());
        HardScience.INSTANCE.info("bits1 length = "  + ModelStateData.PACKER_1.bitLength());
        HardScience.INSTANCE.info("bits2 length = "  + ModelStateData.PACKER_2.bitLength());
        
        HardScience.INSTANCE.info("bits3 block length = "  + ModelStateData.PACKER_3_BLOCK.bitLength());
        HardScience.INSTANCE.info("bits3 flow length = "  + ModelStateData.PACKER_3_FLOW.bitLength());
        ...
        state.setTexture(PaintLayer.BASE, grondag.exotic_matter.init.ModTextures.BLOCK_NOISE_STRONG);
        state.setTexture(PaintLayer.LAMP, grondag.exotic_matter.init.ModTextures.BLOCK_COBBLE);
        state.setTexture(PaintLayer.MIDDLE, ModTextures.DECAL_DIAGONAL_CROSS_BARS);
        state.setTexture(PaintLayer.OUTER, ModTextures.BORDER_SMOOTH_BLEND);
        ...
        assert(reloadedState.getTexture(PaintLayer.BASE) == grondag.exotic_matter.init.ModTextures.BLOCK_NOISE_STRONG);
        assert(reloadedState.getTexture(PaintLayer.LAMP) == grondag.exotic_matter.init.ModTextures.BLOCK_COBBLE);
        assert(reloadedState.getTexture(PaintLayer.MIDDLE) == ModTextures.DECAL_DIAGONAL_CROSS_BARS);
        assert(reloadedState.getTexture(PaintLayer.OUTER) == ModTextures.BORDER_SMOOTH_BLEND);
