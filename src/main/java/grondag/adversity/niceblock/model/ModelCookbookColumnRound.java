...
public class ModelCookbookColumnRound extends ModelCookbookColumnSquare implements ICollisionHandler {
	...
	public int getModelRecipeID(IExtendedBlockState state, IBlockAccess worldIn, BlockPos pos) {
		 ...
		 * occlusion culling on them automatically, so select a model without
		 * them if an adjacent block would occlude them. Because columns are
		 * commonly stacked along an axis, we test for them specifically in
		 * addition to normal cubes, but because model shape varies for columns,
