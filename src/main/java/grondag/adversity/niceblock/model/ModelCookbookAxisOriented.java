...
/**
 * Cookbook for column blocks and other blocks that are stacked along an axis.
 * See ModelCookbook for overview.
 */
public abstract class ModelCookbookAxisOriented extends ModelCookbook implements ICollisionHandler {
	...
	/**
	 * Recipe lookups for axis-aligned lookups are different from base cookbook.
	 */
	protected final Integer[][][][][][] RECIPE_LOOKUP = new Integer[2][2][2][2][2][2];
	
	/**
	 * Transformed version of ROTATION_LOOKUP_Y for the axis of this instance.
	 * Obviously, no transformation needed for Y-axis instance.
	 */
	...
	/**
	 * Maps which base model (out of 18) to use with each recipe index (out of
	 * 64). Also used to map bounding boxes for each recipe because these
	...
	protected void setModelBoundsForRecipe(int recipe) {
		MODEL_BOUNDS[recipe] = getModelBounds(MODEL_FOR_RECIPE[recipe], ROTATION_LOOKUP[recipe].getMatrix());
