...
public class NiceCookbook {
	...
	protected static final TextureOffset[][] SIMPLE_JOIN_MASONRY_OFFSETS = new TextureOffset[4][64];
	/** dimensions are rotation and recipe */
	...
	/** 
	 * retrieves alternate index from expanded value
	 * see calcExpanded 
	 */
	...
	/** 
	 * retrieves rotation from expanded value
	 * see calcExpanded 
	 */
    ...
    /**
	 * Lightweight data structure to store relative position of the texture for each face of a recipe.
	 * Positions are relative to the base texture index (which is not known to this class.)
	 */
	...
	/** 
	 * Texture rotations.  
	 * Used mainly when rotated textures are used as alternate textures.
	 */
	...
	/**
	 * Dynamically determines the correct recipe to use when corner tests matter.
	 * Used specifically in the case of connected textures with outside borders.
	 */
	protected static class CornerRecipeFinder{
		...
		/**
		 * This class assumes your recipes are ordered a particular way without gaps.
		 * The first recipe (identified in the first parameter) is used when all corner blocks are present 
		 * Each missing corner block toggles a bit that is added to the offset.
		 * So, for example, one corner gives two potential outputs, two gives four, etc.
		 * 
		 * Corners should be provided in LSB-first order.
		 * Corner directions should be provided in pairs ordered so that U or D < N or S < E or W.
		 * 
		 * You can use this class without any corner tests.
		 * Do this for consistency of types in lookup arrays when some cases have no corner tests.
		 * 
		 * @param baseRecipeID  Recipe to use if all corners are present.
		 * @param corners  Corners that modify this recipe.
		 */
	...
	static{
		...
		//TODO - need to add texture offsets for masonry
