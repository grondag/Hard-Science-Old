...
//            Log.info(String.format("%s -> %s", face.toString(), FACE_LOOKUP[key][face.ordinal()].toString()));
     ...
     */
    public static Matrix4f getMatrix4f(ModelState modelState)
    {
     ...
     */
    public static EnumFacing rotateFace(@Nonnull ModelState modelState, @Nonnull EnumFacing face)
    {
     ...
     * 
     * Equivalently, list containing results of calling {@link #rotateFace(ModelState, EnumFacing)}
     * for each face in {@link EnumFacing#values()} in order.
     */
    public static FaceMap getFaceMap(@Nonnull ModelState modelState)
    {
        return FACE_MAPS[computeTransformKey(modelState)];
    }
    
    /**
     * Same as {@link #getFaceMap(ModelState)} but uses the output
     * of {@link #computeTransformKey(ModelState)} instead of modelstate.
     */
